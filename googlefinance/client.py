...
def load_trbc():
    try:
        return pd.read_pickle('bin/TRBC-Codes.p')
    except FileNotFoundError as e:
        print(e)


def lookup_trbc(target,column="Sector"):
    trbc = load_trbc()
    qstr = "{0:s} == '{1:s}'".format(column, target)
    # results=trbc.query("Industry_Group == 'Coal'")['Industry_Group_ID'][0]
    return trbc.query(qstr)


def get_by_trbc(codes):
    url = "https://finance.google.com/finance?output=json&q="
    param = lambda x: {"catid": "TRBC:%d" % x}
    headers = {'Accept': 'application/json'}
    path = 'data/'

    for code in codes:
        r = requests.get(url, headers=headers, params=param(code))
        print(r.status_code)
        if r.status_code == 200:
            dat = json.loads(r.text[4:])[0]
            # print(dat['name'], *[x['name'] for x in dat['subcategories']], sep=', ')
            try:
                print(*[(x['id'], x['name'], x['t']) for x in dat['gainers']], sep='\n')
            except KeyError as ke:
                print("KeyError: %s, skipping" % ke)
...
def get_price_data(query):
    ...
    # r = requests.get("getprices", params=query)
...
if __name__ == '__main__':
    '''
    Obviously need to ignore case for lookup. A lot to do here to parse everything correctly. 
    '''
    sw = lookup_trbc("Internet Services", "Industry")
    print(sw[["Industry","Industry_ID","Activity","Activity_ID"]])
    get_by_trbc(sw['Activity_ID'])
